...
promocijski_material: Dict[str, List[dict]] = {
...
cenik_dtf: Dict[int, Tuple[float, float]] = {
    1: (27, 38),
    2: (40, 56),
    4: (72, 101),
    5: (90, 126),
...
def poisci_razpon(cenik: List[dict], kolicina: int, ime_izdelka: Optional[str] = None) -> Optional[dict]:
    if ime_izdelka:
        for razpon in cenik:
            if razpon["min"] <= kolicina <= razpon["max"] and razpon.get("ime", "").lower() == ime_izdelka:
                return razpon
    else:
        for razpon in cenik:
...
def interpoliraj_ceno(metri: float, cenik: Dict[int, Tuple[float, float]]) -> Tuple[float, float]:
...
def save_to_file(podjetje: str, data: List[str], kolicina_or_metri: float, izbira: str, mapa: str = "izracuni_dtf") -> Optional[str]:
